---
name: handoff-codex-with-advice
...
This form grants the Codex worker one attenuated operation: nested read-only `advice`. The worker may use the nested advice command without `--caller-harness` or root budgets. It cannot invoke `run`, `run-with-advice`, or a provider CLI, and advice can never grant write authority.
