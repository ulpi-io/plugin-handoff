---
name: handoff-cursor
description: Delegate a bounded build, phase, review, verify task to Cursor without giving that worker any nested Handoff capability. Use when Cursor should execute independently and must not consult another model through Handoff.
---
...
This is the plain form. The Cursor worker receives no supervisor context and cannot invoke nested advice, `run`, `run-with-advice`, or a provider CLI. Do not silently upgrade this request to the with-advice family.
