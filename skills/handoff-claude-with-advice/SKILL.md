---
name: handoff-claude-with-advice
description: Delegate a bounded build, phase, review, verify task to Claude while allowing that worker to request nested read-only advice. Use when Claude should execute the task and may consult another model without delegating more work.
---

# Hand off to Claude and let it ask for advice
...
This form grants the Claude worker one attenuated operation: nested read-only `advice`. The worker may use the nested advice command without `--caller-harness` or root budgets. It cannot invoke `run`, `run-with-advice`, or a provider CLI, and advice can never grant write authority.
